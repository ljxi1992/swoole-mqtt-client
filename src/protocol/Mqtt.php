...
class Mqtt
{
    ...
    public static function encode(array $data)
    {
        ...
        switch ($cmd) {
            ...
            // ['cmd'=>x, 'message_id'=>x]
            case static::PUBACK:
                $package=self::encodeMessageId($cmd,$data);
                return $package;
            case static::PUBREC:
                $package=self::encodeMessageId($cmd,$data);
                return $package;
            ...
            case static::SUBSCRIBE:
                ...
                return $head . $body;
            // ['cmd'=>9, 'message_id'=>x, 'payload'=>[x,x,..]]
            case static::SUBACK:
        ...
        try {
            ...
            switch ($cmd) {
                ...
                case static::PUBLISH:
                    ...
                    return $package;
                case static::PUBACK:
                    $data=self::decodeMessageId($cmd,$body);
                    return $data;
                case static::PUBREC:
                    $data=self::decodeMessageId($cmd,$body);
                    return $data;
    ...
    public static function getBodyLength($buffer, &$headBytes)
    {
        ...
        do {
            //如果剩余长度不存在，则返回消息体为空
            if (!isset($buffer[$headBytes])) {
                $headBytes = 0;
                return 0;
            }
            //消息体长度
            $digit = ord($buffer[$headBytes]);
            //去掉延续位
     ...
     * @return string
     */
    protected static function encodeMessageId($cmd,$data)
    {
        $body = pack('n', $data['message_id']);
        $head = static::packHead($cmd, strlen($body));
     ...
     * @return string
     */
    protected static function decodeMessageId($cmd,$body)
    {
        $messageId = static::readShortInt($body);
